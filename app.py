...
@st.cache_data
def load_data(path):
    df_loc  = pd.read_excel(path, sheet_name='시추주상도 위치')
    df_info_raw = pd.read_excel(path, sheet_name='시추주상도 정보', header=0)
    df_pile = pd.read_excel(path, sheet_name='말뚝 정보')
    # 정보 시트 전처리
...
df_loc, df_pts, df_pile = load_data('Engineering_tool_3차원지반모델링.xlsx')
