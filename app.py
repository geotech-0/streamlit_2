...
def load_data(file_buffer):
    ...
    return df_loc, df_pts, df_pile

df_loc, df_pts, df_pile = load_data(uploaded_file)
...
for x0, y0, z_top in zip(Xp, Yp, Zp):
    ...
    cylinder_traces.append(go.Surface(
        x=x_grid.T, y=y_grid.T, z=z_grid.T,
        surfacecolor=np.full_like(z_grid.T, zi[idx]),
...
# 4.4 보어홀(시추공) 원통
for bh, group in df_pts.groupby('BH'):
    x0 = group['X'].iloc[0]
    y0 = group['Y'].iloc[0]
    z_top = group['top_z'].iloc[0]
    z_bot = group['Z'].min()
    z_cyl = np.linspace(z_bot, z_top, 2)
    theta_grid, z_grid = np.meshgrid(theta, z_cyl)
    x_grid = x0 + bh_radius * np.cos(theta_grid)
    y_grid = y0 + bh_radius * np.sin(theta_grid)
    cylinder_traces.append(go.Surface(
        x=x_grid.T, y=y_grid.T, z=z_grid.T,
        surfacecolor=np.full_like(z_grid.T, zi[idx]),
...
# 4.5 Figure 조립
fig = go.Figure(data=[vol, slice_surf] + cylinder_traces)
fig.update_layout(
    scene=dict(
        xaxis_title='X (m)',
        yaxis_title='Y (m)',
